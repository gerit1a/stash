...
export const ScenePlayerScrubber: React.FC<IScenePlayerScrubberProps> = (
  ...
  useEffect(() => {
    ...
    el.addEventListener("touchstart", onTouchDown, false);
    return () => {
      if (!el) {
        return;
      }
  ...
  function onMouseUp(this: Window, event: MouseEvent) {
    ...
    handleInputUp(event.target, event.clientX, startMouseEvent.current.clientX, event.offsetX, mouseVelocity.current);
  }
  ...
  function onTouchEnd(this: Window, event: TouchEvent) {
    ...
    handleInputUp(event.target, event.changedTouches[0].clientX, startTouchEvent.current.changedTouches[0].clientX, event.changedTouches[0].screenX, touchVelocity.current);
  }

  function handleInputUp(target: EventTarget | null, inputX: number, startX: number, offsetX: number, velocity: number) {
    if (!scrubberSliderEl.current) {
      return;
    }
  ...
  function onTouchMove(this: HTMLDivElement, event: TouchEvent) {
    ...
    // negative dragging right (past), positive left (future)
    const delta = event.changedTouches[0].clientX - (lastTouchEvent.current?.changedTouches[0].clientX ?? 0);
